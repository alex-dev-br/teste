package br.com.fiap.restaurantusersapi.application.ports.inbound.update;

import br.com.fiap.restaurantusersapi.application.domain.user.Address;

public record UpdateAddressOutput(Long id, String street, String number, String complement, String city, String neighborhood, String state, String zipCode) {
    public UpdateAddressOutput(Address address) {
        this(address.id(), address.street(), address.number(), address.complement(), address.city(), address.neighborhood(), address.state(), address.zipCode());
    }
}
